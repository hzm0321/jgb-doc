---
sidebar_position: 2
title: 基金数据与行情查询
sidebar_label: 2. 基金查询 (fund)
---

# 基金数据与行情查询

本章收录所有面向单只或多只公募基金的基本面查询、实时估值监控、历史区间净值、重仓股与分红明细指令。所有指令均遵循双重输出标准，既可通过表格（Table）或自然语言（Text）在终端阅读，亦可切换为标准结构化 `JSON` 供脚本或 AI Agent 自动解析。

---

## 2.1 `jgb search` —— 搜索基金

#### 语法 (Synopsis)
```bash
jgb search <keyword> [options]
```

#### 描述 (Description)
根据输入的关键词（支持基金代码模糊匹配、名称拼音或汉字关键词），从天天基金全网公募基金名录中全文检索匹配的基金列表。

#### 参数与选项 (Arguments & Options)
| 参数/选项 | 类型 | 是否必填 | 说明 |
| :--- | :--- | :--- | :--- |
| `<keyword>` | string | 是 | 基金代码或名称模糊匹配关键词（如 `005827`、`易方达`、`蓝筹`） |
| `--json` | boolean | `false` | 以结构化 JSON 格式输出结果，同时禁用所有交互式 UI 动画与引导文字，适用于 jq、管道操作及程序化自动化处理 |
| `--text` | boolean | `false` | 以纯文本格式输出结果，去除控制台颜色和特殊 UI 符号，生成结构清晰的文字摘要，专为 LLM / AI Agent 优化消费 |

#### 使用示例 (Examples)
```bash
# 按基金名称关键词搜索
jgb search "易方达蓝筹"

# AI Agent 自动化集成查询 JSON
jgb search "沪深300" --json
```

#### 输出格式 (Output)
**默认表格输出 (`table`)**：
```
🔍 正在搜索 "易方达蓝筹"...
✔ [005827] 易方达蓝筹精选混合 (混合型-偏股)
✔ [009342] 易方达优质精选三年持有期 (混合型-偏股)
✔ [110010] 易方达价值成长混合 (混合型-偏股)
共找到 3 个结果。
```

**JSON 结构化输出 (`--json`)**：
```json
{
  "ok": true,
  "command": "search",
  "timestamp": "2026-06-16T10:30:00+08:00",
  "data": [
    { "code": "005827", "name": "易方达蓝筹精选混合", "category": "混合型-偏股" },
    { "code": "009342", "name": "易方达优质精选三年持有期", "category": "混合型-偏股" },
    { "code": "110010", "name": "易方达价值成长混合", "category": "混合型-偏股" }
  ],
  "meta": {
    "total": 3
  }
}
```

**常规文本输出 (`--text`)**：
```
基金代码: 005827
基金名称: 易方达蓝筹精选混合
基金类别: 混合型-偏股

基金代码: 009342
基金名称: 易方达优质精选三年持有期
基金类别: 混合型-偏股

基金代码: 110010
基金名称: 易方达价值成长混合
基金类别: 混合型-偏股
```

---

## 2.2 `jgb info` —— 查询单基金实时估值与基本面综述

#### 语法 (Synopsis)
```bash
jgb info <code> [options]
```

#### 描述 (Description)
获取指定单只基金的综合数据综述，包括最新官方单位净值、净值确认日期、昨日涨跌幅、日内实时估算净值与估算涨跌幅、估算更新时间，以及底层数据源引用。

#### 参数与选项 (Arguments & Options)
| 参数/选项 | 类型 | 是否必填 | 说明                                                             |
| :--- | :--- | :--- |:---------------------------------------------------------------|
| `<code>` | string | 是 | 6 位有效公募基金代码（如 `005827`）                                        |
| `--source <type>` | number | 否 | 强制指定估值数据源（`1`=天天基金[默认]，`2`=新浪估算，`3`=新浪估算2，`4`=QDII表，`5`=小倍养基xb[PRO 专享]）          |
| `--json` | boolean | `false` | 以结构化 JSON 格式输出结果，同时禁用所有交互式 UI 动画与引导文字，适用于 jq、管道操作及程序化自动化处理     |
| `--text` | boolean | `false` | 以纯文本格式输出结果，去除控制台颜色和特殊 UI 符号，生成结构清晰的文字摘要，专为 LLM / AI Agent 优化消费 |

#### 使用示例 (Examples)
```bash
# 查询基金实时估值与净值信息
jgb info 005827

# 切换至新浪估算接口查询并输出纯文本摘要（适合大模型读取）
jgb info 005827 --source 2 --text

# 使用小倍养基(xb)数据源查询（PRO 会员专享）
jgb info 005827 --source 5
```

#### 输出格式 (Output)
**默认表格输出 (`table`)**：
```
$ jgb info 005827

易方达蓝筹精选混合 (005827)
────────────────────────────────────────
最新净值:    1.5258    (2026-06-18)
最新涨幅:    -0.60%
估算净值:    1.5046
估算涨幅:    -1.98%    (2026-06-18 15:00)
数据来源:    fundgz
```

**JSON 结构化输出 (`--json`)**：
```json
{
  "ok": true,
  "command": "info",
  "timestamp": "2026-06-21T07:23:55.904Z",
  "data": {
    "code": "005827",
    "name": "易方达蓝筹精选混合",
    "dwjz": 1.5258,
    "jzrq": "2026-06-18",
    "zzl": -0.6,
    "gsz": 1.5046,
    "gszzl": -1.98,
    "gztime": "2026-06-18 15:00",
    "noValuation": false,
    "valuationSource": "fundgz"
  }
}
```

**常规文本输出 (`--text`)**：
```
基金名称: 易方达蓝筹精选混合
基金代码: 005827
最新净值: 1.5258
净值日期: 2026-06-18
最新涨幅: -0.60%
估算净值: 1.5046
估算涨幅: -1.98%
估算时间: 2026-06-18 15:00
数据来源: fundgz
```

---

## 2.3 `jgb history` —— 历史区间净值查询

#### 语法 (Synopsis)
```bash
jgb history <code> [options]
```

#### 描述 (Description)
获取指定基金在某一历史时间区间内的每日官方净值（单位净值、累计净值、日增长率）明细记录。

#### 参数与选项 (Arguments & Options)
| 参数/选项 | 类型 | 是否必填 | 说明 |
| :--- | :--- | :--- | :--- |
| `<code>` | string | 是 | 6 位基金代码 |
| `--from <date>` | string | 是 | 查询起始日期，格式 `YYYY-MM-DD`（如 `2026-06-01`） |
| `--to <date>` | string | 否 | 查询结束日期，默认值为今天 |
| `--json` | boolean | `false` | 以结构化 JSON 格式输出结果，同时禁用所有交互式 UI 动画与引导文字，适用于 jq、管道操作及程序化自动化处理 |
| `--text` | boolean | `false` | 以纯文本格式输出结果，去除控制台颜色和特殊 UI 符号，生成结构清晰的文字摘要，专为 LLM / AI Agent 优化消费 |

#### 使用示例 (Examples)
```bash
# 查询 2026 年 6 月全月净值流水
jgb history 005827 --from 2026-06-01 --to 2026-06-30
```

#### 输出格式 (Output)
**默认表格输出 (`table`)**：
```
日期         单位净值    累计净值    涨跌幅
──────────────────────────────────────────────────
2026-06-10   1.5558     1.5558     +0.08%
2026-06-11   1.5514     1.5514     -0.28%
2026-06-12   1.5575     1.5575     +0.39%

共 3 条记录
```

**JSON 结构化输出 (`--json`)**：
```json
{
  "ok": true,
  "command": "history",
  "timestamp": "2026-06-21T07:44:24.350Z",
  "data": [
    { "date": "2026-06-10", "nav": 1.5558, "accNav": 1.5558, "growth": 0.08 },
    { "date": "2026-06-11", "nav": 1.5514, "accNav": 1.5514, "growth": -0.28 },
    { "date": "2026-06-12", "nav": 1.5575, "accNav": 1.5575, "growth": 0.39 }
  ],
  "meta": { "total": 3 }
}
```

**常规文本输出 (`--text`)**：
```
日期: 2026-06-10  单位净值: 1.5558  累计净值: 1.5558  涨跌幅: +0.08%
日期: 2026-06-11  单位净值: 1.5514  累计净值: 1.5514  涨跌幅: -0.28%
日期: 2026-06-12  单位净值: 1.5575  累计净值: 1.5575  涨跌幅: +0.39%
```

---

## 2.4 `jgb returns` —— 阶段涨跌幅统计

#### 语法 (Synopsis)
```bash
jgb returns <code> [options]
```

#### 描述 (Description)
计算指定基金在近 1 周、近 1 月、近 3 月、近 6 月、今年来（YTD）、近 1 年及成立以来的各阶段区间表现百分比数据。

#### 参数与选项 (Arguments & Options)
| 参数/选项 | 类型 | 是否必填 | 说明 |
| :--- | :--- | :--- | :--- |
| `<code>` | string | 是 | 6 位基金代码 |
| `--json` | boolean | `false` | 以结构化 JSON 格式输出结果，同时禁用所有交互式 UI 动画与引导文字，适用于 jq、管道操作及程序化自动化处理 |
| `--text` | boolean | `false` | 以纯文本格式输出结果，去除控制台颜色和特殊 UI 符号，生成结构清晰的文字摘要，专为 LLM / AI Agent 优化消费 |

#### 使用示例 (Examples)
```bash
jgb returns 005827
```

#### 输出格式 (Output)
**默认表格输出 (`table`)**：
```
阶段         涨跌幅
─────────────────────────
近1周        -1.65%
近1月        -7.03%
近3月        -18.10%
近6月        -19.60%
近1年        -13.27%
连跌天数     3 天
```

**JSON 结构化输出 (`--json`)**：
```json
{
  "ok": true,
  "command": "returns",
  "timestamp": "2026-06-21T08:07:01.106Z",
  "data": {
    "code": "005827",
    "week": -1.65,
    "month": -7.03,
    "month3": -18.10,
    "month6": -19.60,
    "year1": -13.27,
    "consecutiveTrend": { "type": "down", "days": 3 }
  }
}
```

**常规文本输出 (`--text`)**：
```
基金代码: 005827
近1周: -1.65%
近1月: -7.03%
近3月: -18.10%
近6月: -19.60%
近1年: -13.27%
连跌天数: 3 天
```

---

## 2.5 `jgb trend` —— 查询基金走势图数据

#### 语法 (Synopsis)
```bash
jgb trend <code> [options]
```

#### 描述 (Description)
获取指定基金在当天交易时段（9:30-15:00）或近期多天的日内每分钟估值变动采样序列点。

#### 参数与选项 (Arguments & Options)
| 参数/选项             | 类型 | 是否必填 | 说明 |
|:------------------| :--- | :--- | :--- |
| `<code>`          | string | 是 | 6 位基金代码 |
| `--range <range>` | string | 否 | 走势跨度，可选范围有 `1m`（近一月）、`3m`（近三月）、`6m`（近六月）、`1y`（近一年）、`3y`（近三年）、`all`（成立以来），默认 `1m` |
| `--json`          | boolean | `false` | 以结构化 JSON 格式输出结果，同时禁用所有交互式 UI 动画与引导文字，适用于 jq、管道操作及程序化自动化处理 |
| `--text`          | boolean | `false` | 以纯文本格式输出结果，去除控制台颜色和特殊 UI 符号，生成结构清晰的文字摘要，专为 LLM / AI Agent 优化消费 |

#### 使用示例 (Examples)
```bash
# 获取近一月净值走势数据
jgb trend 005827 --range 1m
```

#### 输出格式 (Output)
**默认表格输出 (`table`)**：
```
基金代码: 005827
时间范围: 1m
────────────────────────────────────────────────────────────
日期         净值      涨跌幅
───────────────────────────────────
2026-05-24   1.6210   +0.16%
2026-05-25   1.6074   -0.84%
2026-05-26   1.6052   -0.14%
...
2026-06-17   1.5258   -0.60%

共 19 个交易日
累计涨幅: -5.87%
```

**JSON 结构化输出 (`--json`)**：
```json
{
  "ok": true,
  "command": "trend",
  "timestamp": "2026-06-21T08:21:15.488Z",
  "data": {
    "code": "005827",
    "range": "1m",
    "points": [
      { "date": "2026-05-24", "value": 1.6210, "equityReturn": 0.16 },
      { "date": "2026-05-25", "value": 1.6074, "equityReturn": -0.84 },
      { "date": "2026-06-17", "value": 1.5258, "equityReturn": -0.60 }
    ],
    "benchmark": [
      { "name": "沪深300", "points": [
        { "date": "2026-05-24", "value": 3850.00 },
        { "date": "2026-05-25", "value": 3863.48 }
      ]}
    ],
    "totalReturn": -5.87
  }
}
```

**常规文本输出 (`--text`)**：
```
基金代码: 005827
时间范围: 1m
数据点数: 19
累计涨幅: -5.87%
起始日期: 2026-05-24
结束日期: 2026-06-17
起始净值: 1.6210
结束净值: 1.5258
```



---

## 2.6 `jgb holdings` —— 基金重仓股明细

#### 语法 (Synopsis)
```bash
jgb holdings <code> [options]
```

#### 描述 (Description)
查询基金最新定期报告公布的前十大重仓股票列表、标的代码、占资产总值比例、持股数以及较上一报告期的增减变动。

#### 参数与选项 (Arguments & Options)
| 参数/选项 | 类型 | 是否必填 | 说明 |
| :--- | :--- | :--- | :--- |
| `<code>` | string | 是 | 6 位基金代码 |
| `--json`          | boolean | `false` | 以结构化 JSON 格式输出结果，同时禁用所有交互式 UI 动画与引导文字，适用于 jq、管道操作及程序化自动化处理 |
| `--text`          | boolean | `false` | 以纯文本格式输出结果，去除控制台颜色和特殊 UI 符号，生成结构清晰的文字摘要，专为 LLM / AI Agent 优化消费 |

#### 使用示例 (Examples)
```bash
jgb holdings 005827
```

#### 输出格式 (Output)
**默认表格输出 (`table`)**：
```
基金代码: 005827
报告日期: 2026-03-31
────────────────────────────────────────────────────────────
重仓股 (前10):
序号  代码      名称        占净值比    当日涨跌
──────────────────────────────────────────────────
1     600519   贵州茅台   9.91    % -2.02%
2     000858   五粮液     9.90    % -2.12%
3     000568   泸州老窖   9.87    % -2.42%
4     00700    腾讯控股   9.60    % -1.17%
5     09987    百胜中国   9.56    % -0.24%
6     00883    中国海洋石油 9.28    % -2.01%
7     600809   山西汾酒   9.27    % -3.76%
8     09988    阿里巴巴-W 9.22    % -1.87%
9     06618    京东健康   4.39    % -0.90%
10    002027   分众传媒   3.96    % -1.95%
```

**JSON 结构化输出 (`--json`)**：
```json
{
  "ok": true,
  "command": "holdings",
  "timestamp": "2026-06-21T08:54:52.677Z",
  "data": {
    "code": "005827",
    "reportDate": "2026-03-31",
    "isLatestQuarter": true,
    "topHoldings": [
      { "rank": 1, "stockCode": "600519", "stockName": "贵州茅台", "weight": 9.91, "change": -2.02 },
      { "rank": 2, "stockCode": "000858", "stockName": "五粮液", "weight": 9.90, "change": -2.12 },
      { "rank": 3, "stockCode": "000568", "stockName": "泸州老窖", "weight": 9.87, "change": -2.42 }
    ],
    "assetAllocation": []
  }
}
```

**常规文本输出 (`--text`)**：
```
基金代码: 005827
报告日期: 2026-03-31
是否最新季度: 是

重仓股 (前10):
1. 600519 贵州茅台 9.91% -2.02%
2. 000858 五粮液 9.90% -2.12%
3. 000568 泸州老窖 9.87% -2.42%
4. 00700 腾讯控股 9.60% -1.17%
5. 09987 百胜中国 9.56% -0.24%
6. 00883 中国海洋石油 9.28% -2.01%
7. 600809 山西汾酒 9.27% -3.76% -
8. 09988 阿里巴巴-W 9.22% -1.87%
9. 06618 京东健康 4.39% -0.90%
10. 002027 分众传媒 3.96% -1.95%
```

---

## 2.7 `jgb dividends` —— 历史分红记录

#### 语法 (Synopsis)
```bash
jgb dividends <code> [options]
```

#### 描述 (Description)
查询基金成立以来的所有分红派息记录，包括权益登记日、除息日、派息日及每份分红金额。

#### 参数与选项 (Arguments & Options)
| 参数/选项 | 类型 | 是否必填 | 说明 |
| :--- | :--- | :--- | :--- |
| `<code>` | string | 是 | 6 位基金代码 |
| `--since <date>` | string | 否 | 过滤某日期之后的分红记录 |
| `--json`          | boolean | `false` | 以结构化 JSON 格式输出结果，同时禁用所有交互式 UI 动画与引导文字，适用于 jq、管道操作及程序化自动化处理 |
| `--text`          | boolean | `false` | 以纯文本格式输出结果，去除控制台颜色和特殊 UI 符号，生成结构清晰的文字摘要，专为 LLM / AI Agent 优化消费 |

#### 使用示例 (Examples)
```bash
jgb dividends 005827 --since 2024-01-01
```

#### 输出格式 (Output)
**默认表格输出 (`table`)**：
```
日期         每份分红    当日净值
───────────────────────────────────
2026-06-18   0.1063     1.0188

共 1 条记录
```

**JSON 结构化输出 (`--json`)**：
```json
{
  "ok": true,
  "command": "dividends",
  "timestamp": "2026-06-21T09:26:10.759Z",
  "data": [
    { "date": "2026-06-18", "dividend": 0.1063, "nav": 1.0188 }
  ],
  "meta": { "total": 1 }
}
```

**常规文本输出 (`--text`)**：
```
日期: 2026-06-18  每份分红: 0.1063  当日净值: 1.0188
```

---

## 2.8 `jgb ranking` —— 获取全市场基金估值排行

#### 语法 (Synopsis)
```bash
jgb ranking [options]
```

#### 描述 (Description)
按日内估值涨幅、某历史阶段涨跌幅等指标，获取全网公募基金或特定分类基金的榜单排行。

#### 参数与选项 (Arguments & Options)
| 参数/选项 | 类型 | 是否必填 | 说明 |
| :--- | :--- | :--- | :--- |
| `--sort <field>` | string | 否 | 3=估值涨幅(默认), 4=成交热度, 5=实际涨幅 |
| `--order <dir>`  | string | 否 | desc(默认) / asc |
| `--page <page>`  | string | 否 | 页码，默认 1 |
| `--size <size>`  | string | 否 | 每页条数，默认 20 |
| `--json`          | boolean | `false` | 以结构化 JSON 格式输出结果，同时禁用所有交互式 UI 动画与引导文字，适用于 jq、管道操作及程序化自动化处理 |
| `--text`          | boolean | `false` | 以纯文本格式输出结果，去除控制台颜色和特殊 UI 符号，生成结构清晰的文字摘要，专为 LLM / AI Agent 优化消费 |

#### 使用示例 (Examples)
```bash
# 查询今日全网估算涨幅前 5 名
jgb ranking --size 5

# 查询今年来混合型基金表现后 10 名（领跌榜）
jgb ranking --order asc --size 10
```

#### 输出格式 (Output)
**默认表格输出 (`table`)**：
```
排名  代码      名称                    估算涨幅
────────────────────────────────────────────────────────────
1     006281   万家人工智能混合A    +6.21%
2     014162   万家人工智能混合C    +6.21%
3     005300   万家成长优选混合C    +5.77%
4     005299   万家成长优选混合A    +5.77%
5     006270   汇安核心成长混合A    +5.34%

共 23533 条记录，当前第 1 页
还有更多数据，使用 --page 2 查看下一页
```

**JSON 结构化输出 (`--json`)**：
```json
{
  "ok": true,
  "command": "ranking",
  "timestamp": "2026-06-21T09:44:29.373Z",
  "data": [
    { "rank": 1, "code": "006281", "name": "万家人工智能混合A", "estimateChange": 6.21, "heat": null },
    { "rank": 2, "code": "014162", "name": "万家人工智能混合C", "estimateChange": 6.21, "heat": null }
  ],
  "meta": { "total": 23533, "page": 1, "limit": 3, "hasMore": true }
}
```

**常规文本输出 (`--text`)**：
```
1. 006281 万家人工智能混合A +6.21%
2. 014162 万家人工智能混合C +6.21%
3. 005300 万家成长优选混合C +5.77%
```

---

## 2.9 `jgb best-source` —— 估值数据源精准度检测

#### 语法 (Synopsis)
```bash
jgb best-source <code> --jzrq <date> --actual-zzl <val> [options]
```

#### 描述 (Description)
对比不同数据源在指定日期的估值准确度，返回最佳数据源编号

#### 参数与选项 (Arguments & Options)
| 参数/选项 | 类型 | 是否必填 | 说明 |
| :--- | :--- | :--- | :--- |
| `<code>` | string | 是 | 6 位基金代码 |
| `--jzrq <date>` | string | 是 | 回测对比的净值确认日，如 `2026-06-18` |
| `--actual-zzl <val>`| number| 是 | 当天官方最终公布的实际涨跌幅（如 `-0.60`） |

#### 使用示例 (Examples)
```bash
jgb best-source 005827 --jzrq 2026-06-18
```

#### 输出格式 (Output)
**默认表格输出 (`table`)**：
```
基金代码: 005827
────────────────────────────────────────
实际涨跌幅:    -0.60%
最佳数据源:    2
昨日准确度:    是

数据源误差对比:
────────────────────────────────────────
天天基金       偏差: 0.85
新浪估算2      偏差: 0.12
新浪估算3      偏差: 0.34
小倍养基(xb)   偏差: 0.08
```

**JSON 结构化输出 (`--json`)**：
```json
{
  "ok": true,
  "command": "best-source",
  "timestamp": "2026-06-21T10:16:02.039Z",
  "data": {
    "code": "005827",
    "actualZzl": -0.6,
    "bestSource": 2,
    "isYesterdayAccuracy": true,
    "comparison": [
      { "source": 1, "name": "天天基金", "deviation": 0.85 },
      { "source": 2, "name": "新浪估算2", "deviation": 0.12 },
      { "source": 3, "name": "新浪估算3", "deviation": 0.34 },
      { "source": 5, "name": "小倍养基(xb)", "deviation": 0.08 }
    ]
  }
}
```

**常规文本输出 (`--text`)**：
```
基金代码: 005827
实际涨跌幅: -0.60%
最佳数据源: 2
昨日准确度: 是

数据源误差对比:
天天基金: 偏差 0.85
新浪估算2: 偏差 0.12
新浪估算3: 偏差 0.34
小倍养基(xb): 偏差 0.08
```

---

## 2.10 `jgb signal` —— 波段信号综合评估

#### 语法 (Synopsis)
```bash
jgb signal <code> [options]
```

#### 描述 (Description)
对指定基金进行波段信号综合评估，输出五大量化子指标（历史分位数、Z-Score、布林带 %B、乖离率 BIAS、RSI）加权合成的综合评分、区域判定与操作建议，并基于信号触发策略进行历史回测，展示买入/卖出信号次数、胜率、策略收益、自然收益（买入持有）及超额收益。回测默认截取近 1 年范围执行，支持通过 `--range` 切换预设范围或 `--from`/`--to` 自定义日期区间。

#### 参数与选项 (Arguments & Options)
| 参数/选项 | 类型 | 是否必填 | 说明 |
| :--- | :--- | :--- | :--- |
| `<code>` | string | 是 | 6 位基金代码（如 `005827`） |
| `--date <date>` | string | 否 | 目标查询日期，格式 `YYYY-MM-DD`（默认最新日期） |
| `--range <range>` | string | 否 | 回测范围：`6m`(近半年) / `1y`(近1年) / `3y`(近3年) / `all`(全部) / `custom`(自定义)，默认 `1y` |
| `--from <date>` | string | 否 | 自定义回测起始日期 `YYYY-MM-DD`，需配合 `--range custom` 使用 |
| `--to <date>` | string | 否 | 自定义回测结束日期 `YYYY-MM-DD`，需配合 `--range custom` 使用，默认最新日期 |
| `--json` | boolean | `false` | 以结构化 JSON 格式输出结果，同时禁用所有交互式 UI 动画与引导文字，适用于 jq、管道操作及程序化自动化处理 |
| `--text` | boolean | `false` | 以纯文本格式输出结果，去除控制台颜色和特殊 UI 符号，生成结构清晰的文字摘要，专为 LLM / AI Agent 优化消费 |

:::note `--range` 与 `--from`/`--to` 联动规则
- 预设范围 `6m`/`1y`/`3y`/`all` 无需 `--from`/`--to`，系统自动以当前日期零点向前推算起始日
- 自定义范围 `--range custom` **必须**配合 `--from` 使用，`--to` 可选（默认到最新日期）
- `--from`/`--to` 仅在 `--range custom` 时生效，与其他预设范围同时使用将报错
- `--from` 不能晚于 `--to`，否则报错
:::

#### 使用示例 (Examples)
```bash
# 查询最新波段信号（默认近1年回测）
jgb signal 005827

# 指定目标日期查询历史信号
jgb signal 005827 --date 2026-07-01

# 切换回测范围为近半年
jgb signal 005827 --range 6m

# 查询近3年完整回测并以 JSON 输出
jgb signal 005827 --range 3y --json

# 自定义日期范围回测
jgb signal 005827 --range custom --from 2025-01-01 --to 2026-06-30

# 自定义起始日期（默认到最新日期）
jgb signal 005827 --range custom --from 2025-06-01

# AI Agent 自动化集成查询纯文本摘要
jgb signal 017193 --text
```

#### 输出格式 (Output)
**默认表格输出 (`table`)**：
```
$ jgb signal 005827

易方达蓝筹精选混合 (005827)
──────────────────────────────────────────────────
目标日期:    2026-08-20
回测范围:    近1年 (2025-08-19 ~ 2026-08-20)
信号总数:    711
──────────────────────────────────────────────────
综合评分:    0.36 / 1.00  [中性区]
──────────────────────────────────────────────────
子指标详情:
  历史分位数:    13.1%
  Z-Score:       -1.50
  布林带 %B:     0.576
  乖离率 BIAS:   0.07%
  RSI:           50.3
──────────────────────────────────────────────────
操作建议:    谨慎观望
建议说明:    处于偏低区间，建议耐心等待
──────────────────────────────────────────────────
策略回测小结 (近1年):
  买入信号:      7 次
  卖出信号:      5 次
  历史胜率:      --
  策略收益:      -17.48%
  自然收益:      -20.91%
  超额收益:      +3.43%
  当前状态:      持仓中（策略收益按最新净值估算）
──────────────────────────────────────────────────
  * 历史回测仅供参考，不构成投资建议
```

**JSON 结构化输出 (`--json`)**：
```json
{
  "ok": true,
  "command": "signal",
  "timestamp": "2026-08-20T15:17:32.661Z",
  "data": {
    "code": "005827",
    "name": "易方达蓝筹精选混合",
    "targetDate": "2026-08-20",
    "backtestRange": "1y",
    "backtestStartDate": "2025-08-19",
    "backtestEndDate": "2026-08-20",
    "signal": {
      "date": "2026-08-20",
      "score": 0.3553,
      "zone": "neutral",
      "indicators": {
        "percentile": 0.1315,
        "zScore": 0.2494,
        "bollingerB": 0.5755,
        "bias": 0.5030,
        "rsi": 0.5028
      }
    },
    "recommendation": {
      "label": "谨慎观望",
      "desc": "处于偏低区间，建议耐心等待",
      "action": "wait"
    },
    "backtest": {
      "buyCount": 7,
      "sellCount": 5,
      "trades": [],
      "winRate": 0,
      "strategyReturn": -17.48,
      "buyHoldReturn": -20.91,
      "excessReturn": 3.43,
      "isHolding": true
    },
    "totalSignals": 711
  }
}
```

**常规文本输出 (`--text`)**：
```
基金代码: 005827
基金名称: 易方达蓝筹精选混合
目标日期: 2026-08-20
回测范围: 近1年
回测区间: 2025-08-19 ~ 2026-08-20
信号总数: 711
综合评分: 0.36
区域判定: 中性区
历史分位数: 13.1%
Z-Score: -1.50
布林带%B: 0.576
乖离率BIAS: 0.07%
RSI: 50.3
操作建议: 谨慎观望
建议说明: 处于偏低区间，建议耐心等待
买入信号: 7 次
卖出信号: 5 次
历史胜率: --
策略收益: -17.48%
自然收益: -20.91%
超额收益: +3.43%
```

#### 字段说明 (Field Reference)

**信号评分 (`signal`)**：

| 字段 | 说明 |
| :--- | :--- |
| `score` | 综合评分（0~1），由五大子指标加权合成，越低表示估值越低 |
| `zone` | 区域判定：`low`(低位区) / `neutral`(中性区) / `high`(高位区) |
| `indicators.percentile` | 历史分位数（0~1），当前净值在过去 252 个交易日中的排名位置 |
| `indicators.zScore` | Z-Score 标准化值（0~1），反映偏离均值的程度 |
| `indicators.bollingerB` | 布林带 %B（0~1），价格在布林带中的相对位置 |
| `indicators.bias` | 乖离率 BIAS（0~1），价格偏离均线的程度 |
| `indicators.rsi` | RSI 相对强弱指标（0~1） |

:::tip 子指标显示范围
在 `--text` 和 `table` 输出中，子指标会还原为原始分析区间以便阅读：
- Z-Score：`(zScore × 6 - 3)`，范围 -3 ~ +3
- BIAS：`(bias × 24 - 12)`，范围 -12% ~ +12%
- RSI：`(rsi × 100)`，范围 0 ~ 100
- 分位数/布林带 %B：原值 × 100%
:::

**操作建议 (`recommendation`)**：

| 评分区间 | action | label | 说明 |
| :--- | :--- | :--- | :--- |
| 0.00 ~ 0.15 | `strongBuy` | 逢低建仓 | 处于极低区间，适合分批建仓 |
| 0.15 ~ 0.30 | `buy` | 买入区间 | 处于偏低区间，可考虑买入 |
| 0.30 ~ 0.45 | `wait` | 谨慎观望 | 处于偏低区间，建议耐心等待 |
| 0.45 ~ 0.55 | `hold` | 中性观望 | 处于中性区间，建议持有观望 |
| 0.55 ~ 0.85 | `sell` | 分批减仓 | 处于偏高区间，建议分批减仓 |
| 0.85 ~ 1.00 | `strongSell` | 止盈离场 | 处于极高区间，建议止盈离场 |

**策略回测 (`backtest`)**：

| 字段 | 说明 |
| :--- | :--- |
| `buyCount` | 回测期间触发的买入信号次数 |
| `sellCount` | 回测期间触发的卖出信号次数 |
| `trades` | 完成的交易记录列表（买入后已卖出的配对交易） |
| `winRate` | 胜率（%），盈利交易数 / 总交易数 |
| `strategyReturn` | 策略总收益率（%），按信号买卖累计计算 |
| `buyHoldReturn` | 自然收益（%），买入持有策略的区间收益率 |
| `excessReturn` | 超额收益（%），`strategyReturn - buyHoldReturn` |
| `isHolding` | 当前是否处于持仓状态（策略最后一次买入后尚未卖出） |

---
