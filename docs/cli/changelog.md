...
- **持仓实时状态支持分组统计**：`jgb holding status` 支持 `--group <分组 ID 或唯一分组名称>`，按指定分组的持仓及交易记录计算实时估值、今日收益和累计盈亏，可与 `--sort`、`--order`、`--json`、`--text` 组合使用。不传 `--group` 时默认汇总全局及所有分组持仓。

### 🐛 问题修复

- **修复分组持仓实时状态为空**：此前 `jgb holding status` 仅统计全局持仓，用户仅在分组中持仓时，会出现 `jgb holding list` 正常显示、实时状态却无明细且汇总为零的问题。现已纳入分组持仓，同一基金跨分组持有时累加份额与总成本，并计算加权成本价。
