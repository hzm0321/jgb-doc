---
sidebar_position: 3
title: 大盘指数与市场行情
sidebar_label: 3. 大盘行情 (market)
---

# 大盘指数与市场行情

本章详细介绍如何通过 CLI 获取全球宏观大盘指数实时报价、A 股法定交易日历状态判断，以及单个基金关联概念板块及其资金流向。此类宏观与行业维度数据非常适合用于大模型智能体进行市场情绪研判和宏观背景归因分析。

---

## 3.1 `jgb market` —— 批量获取全球大盘指数实时行情

#### 语法 (Synopsis)
```bash
jgb market [options]
```

#### 描述 (Description)
批量获取全球主要金融市场核心股票指数的实时行情，默认覆盖 A 股（上证指数、深证成指、创业板指、沪深300、科创50等）、美股（纳斯达克、标普500、道琼斯）、港股（恒生指数、恒生科技）、欧洲核心国家及亚太主要指数。

#### 参数与选项 (Arguments & Options)
| 参数/选项 | 类型 | 是否必填 | 说明 |
| :--- | :--- | :--- | :--- |
| `--filter <market>` | string | 否 | 按市场筛选：`a-share`（仅 A 股）、`us`（美股）、`hk`（港股）、`global`（全海外） |
| `--json` | boolean | `false` | 以结构化 JSON 格式输出结果，同时禁用所有交互式 UI 动画与引导文字，适用于 jq、管道操作及程序化自动化处理 |
| `--text` | boolean | `false` | 以纯文本格式输出结果，去除控制台颜色和特殊 UI 符号，生成结构清晰的文字摘要，专为 LLM / AI Agent 优化消费 |

#### 使用示例 (Examples)
```bash
# 查询全球全部大盘指数行情
jgb market

# 仅筛选查看 A 股核心指数
jgb market --filter a-share

# 在 Shell 脚本或自动化应用中获取 JSON 格式结果
jgb market --filter us --json
```

#### 输出格式 (Output)
**默认表格输出 (`table`)**：
```
$ jgb market --filter a-share

指数              最新点位      涨跌额      涨跌幅
────────────────────────────────────────────────────────────
上证指数            4090.48        -17.60    -0.43%
上证50              2928.75         -5.72    -0.19%
深证成指           16030.70       +149.75    +0.94%
沪深300             4941.60        +10.21    +0.21%
创业板指            4252.39        +85.34    +2.05%
科创50              1911.51        +70.69    +3.84%
```

**JSON 结构化输出 (`--json`)**：
```json
{
  "ok": true,
  "command": "market",
  "timestamp": "2026-06-21T11:55:10.881Z",
  "data": [
    { "name": "上证指数", "code": "sh000001", "price": 4090.48, "change": -17.60, "changePercent": -0.43 },
    { "name": "上证50", "code": "sh000016", "price": 2928.75, "change": -5.72, "changePercent": -0.19 },
    { "name": "深证成指", "code": "sz399001", "price": 16030.70, "change": 149.75, "changePercent": 0.94 }
  ],
  "meta": { "total": 17 }
}
```

**常规文本输出 (`--text`)**：
```
上证指数: 4090.48 -17.60 -0.43%
上证50: 2928.75 -5.72 -0.19%
深证成指: 16030.70 +149.75 +0.94%
沪深300: 4941.60 +10.21 +0.21%
创业板指: 4252.39 +85.34 +2.05%
科创50: 1911.51 +70.69 +3.84%
```

---

## 3.2 `jgb trading-day` —— A 股法定交易日历判断

#### 语法 (Synopsis)
```bash
jgb trading-day [date] [options]
```

#### 描述 (Description)
判断今天或指定的具体日期是否为 A 股法定有效交易日（自动排除周六日及中国法定节假日）。支持传入月份参数以生成完整的月度交易日历明细，便于脚本自动化调度申购、定投或定时任务（如只在交易日 14:50 触发估值提醒脚本）。

#### 参数与选项 (Arguments & Options)
| 参数/选项 | 类型 | 是否必填 | 说明 |
| :--- | :--- | :--- | :--- |
| `[date]` | string | 否 | 指定待检查的具体日期，格式 `YYYY-MM-DD`。默认检查当天 |
| `--month <YYYY-MM>`| string| 否 | 输出指定月份全月的日历状态表（例如 `2026-07`） |
| `--next` | flag | 否 | 寻找指定日期之后的下一个紧邻有效交易日 |
| `--prev` | flag | 否 | 寻找指定日期之前的一个紧邻有效交易日 |
| `--json` | boolean | `false` | 以结构化 JSON 格式输出结果，同时禁用所有交互式 UI 动画与引导文字，适用于 jq、管道操作及程序化自动化处理 |
| `--text` | boolean | `false` | 以纯文本格式输出结果，去除控制台颜色和特殊 UI 符号，生成结构清晰的文字摘要，专为 LLM / AI Agent 优化消费 |

#### 使用示例 (Examples)
```bash
# 检查今天是否为交易日
jgb trading-day

# 检查 2026 年国庆期间某天是否交易日
jgb trading-day 2026-10-01

# 获取 2026 年 7 月完整日历并以 JSON 输出
jgb trading-day --month 2026-07 --json

```

#### 输出格式 (Output)
**默认表格输出 (`table`)**：
```
日期         星期    是否交易日   当前状态            休市原因
────────────────────────────────────────────────────────────────
2026-07-03   周五    是          盘中交易 (14:30:00)  --
```


**月度查询 (`--month`) 表格输出**：
```
2026年7月 A股交易日历
日期         星期    是否交易日   休市原因
─────────────────────────────────────────────────
2026-07-01   周三    是          --
2026-07-02   周四    是          --
2026-07-03   周五    是          --
2026-07-04   周六    否          周末休市
2026-07-05   周日    否          周末休市
```

**JSON 结构化输出 (`--json`)**：
```json
{
  "ok": true,
  "command": "trading-day",
  "timestamp": "2026-07-03T14:30:00.000Z",
  "data": {
    "date": "2026-07-03",
    "dayOfWeek": "Friday",
    "isTradingDay": true,
    "marketStatus": "open",
    "reason": null
  }
}
```

**月度查询 (`--month`) JSON 结构化输出 (`--json`)**：
```json
{
  "ok": true,
  "command": "trading-day",
  "timestamp": "2026-07-03T14:30:00.000Z",
  "data": [
    { "date": "2026-07-01", "dayOfWeek": "Wednesday", "isTradingDay": true, "marketStatus": "open", "reason": null },
    { "date": "2026-07-02", "dayOfWeek": "Thursday", "isTradingDay": true, "marketStatus": "open", "reason": null },
    { "date": "2026-07-03", "dayOfWeek": "Friday", "isTradingDay": true, "marketStatus": "open", "reason": null },
    { "date": "2026-07-04", "dayOfWeek": "Saturday", "isTradingDay": false, "marketStatus": "closed", "reason": "周末休市" },
    { "date": "2026-07-05", "dayOfWeek": "Sunday", "isTradingDay": false, "marketStatus": "closed", "reason": "周末休市" }
  ],
  "meta": {
    "total": 5
  }
}
```

**常规文本输出 (`text`)**：
```
日期: 2026-07-03
星期: 星期五
是否交易日: 是
当前状态: 盘中交易 (14:30:00)
休市原因: --
```

**月度查询 (`--month`) 文本输出 (`--text`)**：
```
日期: 2026-07-01  星期: 周三  是否交易日: 是  休市原因: --
日期: 2026-07-02  星期: 周四  是否交易日: 是  休市原因: --
日期: 2026-07-03  星期: 周五  是否交易日: 是  休市原因: --
日期: 2026-07-04  星期: 周六  是否交易日: 否  休市原因: 周末休市
日期: 2026-07-05  星期: 周日  是否交易日: 否  休市原因: 周末休市
```

---

## 3.3 `jgb sectors` —— 基金关联板块与资金流向

#### 语法 (Synopsis)
```bash
jgb sectors <code> [options]
```

#### 描述 (Description)
查询指定基金在底层投资体系中所关联的行业板块或概念板块，以及这些板块当日的实时涨跌幅和主力资金净流入情况。支持按行业（`industry`）或概念（`concept`）进行筛选，并可按涨跌幅或主力资金净流入灵活进行升降序排列，帮助投资者快速判断该基金盘中净值异动的驱动板块来源。

#### 参数与选项 (Arguments & Options)
| 参数/选项 | 类型 | 是否必填 | 说明 |
| :--- | :--- | :--- | :--- |
| `<code>` | string | 是 | 6 位基金代码 |
| `--type <type>` | string | 否 | 关联板块类型筛选：`industry`（行业板块[默认]）或 `concept`（概念板块） |
| `--sort <field>` | string | 否 | 排序指标：`change`（涨幅[默认]）、`inflow`（资金流入） |
| `--order <dir>` | string | 否 | 排序方向：`desc`（降序[默认]）、`asc`（升序） |
| `--json` | boolean | `false` | 以结构化 JSON 格式输出结果，同时禁用所有交互式 UI 动画与引导文字，适用于 jq、管道操作及程序化自动化处理 |
| `--text` | boolean | `false` | 以纯文本格式输出结果，去除控制台颜色和特殊 UI 符号，生成结构清晰的文字摘要，专为 LLM / AI Agent 优化消费 |


#### 使用示例 (Examples)
```bash
# 查询某基金关联的行业板块（默认按涨幅降序排列）
jgb sectors 005827

# 查询关联的概念板块，并按资金净流入降序排列
jgb sectors 005827 --type concept --sort inflow

# 查询关联的行业板块，按涨幅从低到高（升序）排列
jgb sectors 005827 --type industry --order asc
```

#### 输出格式 (Output)
**默认表格输出 (`table`)**：
```
基金: 易方达蓝筹精选混合 (005827)
关联行业板块明细:
板块名称      板块代码 (secid)   当日涨跌   主力资金净流入 (万元)
──────────────────────────────────────────────────────────
白酒概念      s_sh000031         +1.52%     +125,400.00
消费50        1.BK0577           +0.88%     +45,210.50
食品饮料      s_sh000807         +0.76%     +88,900.00
港股互联网    s_hk000001         -0.45%     -32,100.00
```

**JSON 结构化输出 (`--json`)**：
```json
{
  "ok": true,
  "command": "sectors",
  "data": {
    "fundCode": "005827",
    "sectors": [
      { "name": "白酒概念", "secid": "s_sh000031", "changePercent": 1.52, "mainNetInflow": 125400.00 },
      { "name": "消费50", "secid": "1.BK0577", "changePercent": 0.88, "mainNetInflow": 45210.50 },
      { "name": "食品饮料", "secid": "s_sh000807", "changePercent": 0.76, "mainNetInflow": 88900.00 }
    ]
  }
}
```
**常规文本输出 (`--text`)**：
```
基金: 易方达蓝筹精选混合 (005827)
白酒概念 (s_sh000031): 涨跌 +1.52%  主力净流入 +125,400.00万元
消费50 (1.BK0577): 涨跌 +0.88%  主力净流入 +45,210.50万元
食品饮料 (s_sh000807): 涨跌 +0.76%  主力净流入 +88,900.00万元
港股互联网 (s_hk000001): 涨跌 -0.45%  主力净流入 -32,100.00万元
```

